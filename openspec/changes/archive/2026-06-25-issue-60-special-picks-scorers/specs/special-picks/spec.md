...
### Requirement: Admin pick override

(Reason: The admin override capability was removed in a post-implementation correction round. Admins now manage only their own pick row; the override endpoint, the override modal, and the override audit action are gone with no replacement.)
(Migration: None — `PUT /api/admin/picks/:userId` and `GET /api/admin/picks` are removed; the `pick_override` audit action is no longer emitted. The admin's own post-lock submit/update bypass is preserved as a scenario under "Shared R16 lock for normal users".)

### Requirement: Eye-icon popup visibility

(Reason: The eye-icon popup was replaced by the inline community table in `picksView` in a post-implementation correction round. There is no popup, no modal, and no `updatedBy` column for any viewer.)
(Migration: The "Community picks table visibility" requirement (ADDED) replaces this with an inline table visible to all authenticated users.)
